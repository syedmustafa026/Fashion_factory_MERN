...
const Checkout = ({ setOrderFormValue }) => {
  ...
  const [formValues, setFormValues] = useState({
    firstname: "Mustafa",
    lastname: "Ahmed",
    email: "syedmustafa@gmail.com",
    phonenumber: "033302793679",
    streetaddress: "Street 4 North Nazimabad",
    country: "Pakistan",
    postal: "78493",
    city: "Karachi",
    province: "Sindh",
    card: "42424242424242424242",
    expiration: "12/24",
    cvv: "786",
  });
  ...
  useEffect(() => {
    document.title = "Checkout | Fashion Factory";
  }, []);
  ...
  const submitForm = async (e) => {
    ...
    if (Object.keys(validateForm(formValues)).length > 0) {
      ...
      axios.post("http://localhost:3000/order", formValues).then(
        (response) => {
          console.log("res", response);
          alert("Order Successfully recieved");
          navigate("/order");
        },
        (error) => {
          alert(error);
  ...
  return (
    <main className="checkout">
      ...
      <article className="checkout-content">
        ...
        <form style={{ marginLeft: "20%" }} onSubmit={submitForm}>
          {currentStep === "details" ? (
            <React.Fragment>
